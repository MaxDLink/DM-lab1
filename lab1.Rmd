...
1.  Problem Description (Business Understanding) [10] Describe the Problem: What is COVID-19, and what is social distancing and flattening the curve? Why is it important to look at data about the virus spread, hospitalizations, and available resources? [3 points]
...
Choose a stakeholder for whom you analyze and, later on, model the data. Define some questions that are important for this stakeholder. What decisions can your stakeholders make, and how would they affect COVID-19 outcomes? Brainstorm this a lot since this choice will guide your exploration of this and all the following projects. Make sure you can produce actionable recommendations for these questions using your data later in your report. [7 point]
...
unfiltered_clean_mobility <- clean_mobility_data

head(clean_mobility_data, 10)

# Filter clean_mobility_data to include only Texas in sub_region_1
clean_mobility_data <- clean_mobility_data %>%
  filter(tolower(sub_region_1) == "texas")

# convert date from <chr> to <date> format 
clean_mobility_data$date <- as.Date(clean_mobility_data$date, format = "%Y-%m-%d")

# updated date 
head(clean_mobility_data, 10)

# Write the filtered dataframe to clean_mobility_data.csv
write.csv(clean_mobility_data, "clean_mobility_data.csv", row.names = FALSE)
...
# Grocery value change over time 
cmd <- mobility_data |>
...
retail_and_recreation <- clean_mobility_data %>%
  summarise(
...
Residentia <- clean_mobility_data %>%
  summarise(
...
ggplot(clean_cases_plus_final, aes(x = median_income)) +
  ...
  labs(title = "Distribution of Median Income in Texas Counties",
...
```{r}
# confirmed cases
ggplot(clean_cases_plus_final, aes(y = confirmed_cases)) +
  geom_boxplot(fill = "lightblue") +
  labs(title = "Box Plot of Confirmed Cases in Texas Counties",
       y = "Confirmed Cases") +
  theme_minimal()
...
ggplot(clean_cases_plus_final, aes(y = deaths)) +
  ...
  labs(title = "Box Plot of Deaths in Texas Counties",
       y = "Deaths") +
  theme_minimal()
...
ggplot(clean_cases_plus_final, aes(x = median_age)) +
  ...
  labs(title = "Distribution of Median Age in Texas Counties",
...
ggplot(clean_mobility_data, aes(x = date, y = retail_and_recreation_percent_change_from_baseline)) +
  geom_line(color = "orange") +
  labs(title = "Retail and Recreation Mobility Change Over Time in Texas",
...
ggplot(clean_mobility_data, aes(x = date, y = residential_percent_change_from_baseline)) +
  geom_line(color = "lightblue") +
  labs(title = "Residential Mobility Change Over Time in Texas",
       x = "Date", y = "Percent Change from Baseline") +
  theme_minimal()
...
```{r}
# Assuming your datasets are already loaded as data frames
# For example:
# texas_case <- read.csv("path_to_texas_case.csv")
# clean_mobility_data <- read.csv("path_to_clean_mobility_data.csv")


# sub_region_2 to lower in clean_mobility_data 

clean_mobility_data$sub_region_2 <- tolower(clean_mobility_data$sub_region_2)


# Merge the datasets
merged_data <- texas_cases %>%
  left_join(
    select(clean_mobility_data, sub_region_2, date, retail_and_recreation_percent_change_from_baseline, residential_percent_change_from_baseline),
    by = c("county_name" = "sub_region_2", "date" = "date")
  )

# Now merged_data contains all columns from texas_case plus retail_and_recreation_percent_change
...
merged_data <- subset(merged_data, select = -c(state_fips_code, state, county_fips_code))

# merged_data 10 items 
head(merged_data, 10) 
...
```{r}
# graphs for merged_data 

cor_matrix <- cor(merged_data[, c("confirmed_cases", "deaths", 
                                  "retail_and_recreation_percent_change_from_baseline", 
                                  "residential_percent_change_from_baseline")],
...
ggplot(merged_data, aes(x = retail_and_recreation_percent_change_from_baseline, y = confirmed_cases)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", color = "red") +
  labs(title = "Confirmed Cases vs. Retail & Recreation Mobility Change",
...
merged_data <- merged_data %>% 
  mutate(retail_mobility_group = ifelse(retail_and_recreation_percent_change_from_baseline < -25, 
                                          "high_reduction", "low_reduction"))


ggplot(merged_data, aes(x = retail_mobility_group, y = confirmed_cases)) +
  geom_boxplot(fill = "orange") +
  labs(title = "Confirmed Cases by Retail Mobility Group",
       x = "Retail Mobility Group", 
       y = "Confirmed Cases") +
  theme_minimal()
...
ggplot(merged_data, aes(x = date)) +
  geom_line(aes(y = confirmed_cases), color = "red") +
  geom_line(aes(y = retail_and_recreation_percent_change_from_baseline), color = "blue") +
  scale_y_continuous(name = "Confirmed Cases",
                     sec.axis = sec_axis(~., name = "Retail & Recreation % Change")) +
  labs(title = "Confirmed Cases and Retail & Recreation Mobility Over Time",
       x = "Date") +
  theme_minimal()

library(ggplot2)

ggplot(merged_data, aes(x = confirmed_cases, y = deaths)) +
...
cor(merged_data$confirmed_cases, merged_data$deaths, use = "complete.obs")
...
ccf(merged_data$confirmed_cases, 
    merged_data$residential_percent_change_from_baseline, 
    lag.max = 30, 
    na.action = na.pass)
...
# Categorize retail mobility changes into groups based on a threshold (e.g., -25%)
merged_data <- merged_data %>%
  mutate(retail_mobility_group = ifelse(retail_and_recreation_percent_change_from_baseline < -25,
                                          "high_reduction", "low_reduction"))
...
# Group-wise averages
group_averages <- merged_data %>%
  group_by(retail_mobility_group) %>%
  summarise(avg_cases = mean(confirmed_cases, na.rm = TRUE),
            avg_deaths = mean(deaths, na.rm = TRUE))
print(group_averages)

model <- lm(confirmed_cases ~ retail_and_recreation_percent_change_from_baseline + residential_percent_change_from_baseline, 
            data = merged_data)
summary(model)

library(ggplot2)
ggplot(cases_plus, aes(x = median_income, y = deaths)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", color = "red") +
  labs(title = "Median Income vs. Confirmed Cases",
       x = "Median Income ($)", y = "Confirmed Cases") +
  theme_minimal()
cor(cases_plus$median_income, cases_plus$confirmed_cases, use = "complete.obs")

ggplot(cases_plus, aes(x = median_age, y = deaths)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", color = "blue") +
  labs(title = "Median Age vs. Deaths",
       x = "Median Age (years)", y = "Deaths") +
  theme_minimal()
cor(cases_plus$median_age, cases_plus$deaths, use = "complete.obs")

ggplot(cases_plus, aes(x = confirmed_cases, y = deaths)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", color = "green") +
  labs(title = "Confirmed Cases vs. Deaths",
       x = "Confirmed Cases", y = "Deaths") +
  theme_minimal()
cor(cases_plus$confirmed_cases, cases_plus$deaths, use = "complete.obs")
...
Now we can see how deaths relate to retail_and_recreation_percent_change_from_baseline and residential_percent_change_from_baseline ....
...
# Load the data from the CSV (assuming it's already loaded as tx_data_with_gr)
# If you need to load it manually, you can use:
# tx_data_with_gr <- read.csv("path_to_your_file.csv", stringsAsFactors = FALSE)
...
# Load the data (assuming it's already calculated as texas_cases_with_extended_Growth_Rate)
# If you need to load it manually or recalculate, use the previous code to generate it
# texas_cases_with_extended_Growth_Rate <- ... (your previous code here)
...
ggplot(tx_data_with_gr, aes(x = total_pop, y = avg_growth_rate)) +
  ...
  labs(
    title = "Average Growth Rate vs. Total Population in Texas Counties",
    x = "Total Population",
...
# Prepare tx_data_agg (same as before)
tx_data_agg <- clean_cases_plus_final %>%
  filter(state == "tx") %>%
  group_by(county_name) %>%
  ...
  ungroup() %>%
  mutate(cases_per_100k = (confirmed_cases / total_pop) * 100000)

# Create scatterplot with trend line
ggplot(tx_data_agg, aes(x = median_age, y = cases_per_100k)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  labs(title = "Cases per 100k vs. Median Age in Texas Counties",
       x = "Median Age", y = "Cases per 100,000") +
  theme_minimal()

```
Counties with lower median ages have lower case rates. Older people do not go out as much as younger people. 
...
# Create histogram
ggplot(tx_data_agg, aes(x = cases_per_100k)) +
  geom_histogram(binwidth = 500, fill = "lightblue", color = "black") +
  labs(title = "Distribution of Cases per 100k in Texas Counties",
       x = "Cases per 100,000", y = "Number of Counties") +
  theme_minimal()
```

cases_per_100k = the number of confirmed COVID-19 cases per 100,000 people in each Texas county

number of counties that fall into each bin on the x axis 

This shows the distribution of case rates among counties. For example, 5 counties have about 5,000 cases per 100,000 people

we can also make a scatterplot to label the counties
...
tx_data_agg <- tx_data_agg %>%
  mutate(index = row_number())

# Calculate the 95th percentile threshold
threshold <- quantile(tx_data_agg$cases_per_100k, 0.95, na.rm = TRUE)

# Verify the number of counties in the top 5%
top_counties <- subset(tx_data_agg, cases_per_100k >= threshold)
print(paste("Number of counties in top 5%:", nrow(top_counties)))
print(top_counties)

# Create scatterplot and label top 5% counties
ggplot(tx_data_agg, aes(x = index, y = cases_per_100k)) +
  geom_point(data = subset(tx_data_agg, cases_per_100k < threshold), color = "black") +
  geom_point(data = top_counties, color = "red") +
  geom_text_repel(data = top_counties, aes(label = county_name), size = 3, max.overlaps = 20) +
  labs(title = "Cases per 100k in Texas Counties with Top 5% Highlighted",
       x = "County Index", y = "Cases per 100,000") +
  theme_minimal() +
  ylim(0, max(tx_data_agg$cases_per_100k, na.rm = TRUE) * 1.1)



```
The scatterplot visualizes the cases_per_100k (cases per 100,000 people) for Texas counties, with labels for the top 5% of counties by cases_per_100k.

we can verify that there are about 10 counties labeled to match our threshold. These 10 counties will be our outliers. 
...
# Filter and aggregate tx_data in one step
tx_data_agg <- clean_cases_plus_final %>%
  filter(state == "tx") %>%
  group_by(county_name) %>%
  summarise(confirmed_cases = sum(confirmed_cases, na.rm = TRUE),
...
model <- lm(confirmed_cases ~ median_age + median_income + total_pop, data = tx_data_agg)
summary(model)


```

There could be multiple issues with this model like outliers and skewed results from raw counts etc. so lets make another regression model and fix some of these issues 

```{r}
# Load library
library(dplyr)

# Filter, aggregate, and model in one pipeline with normalization
model <- clean_cases_plus_final %>%
  filter(state == "tx") %>%
  group_by(county_name) %>%
  summarise(confirmed_cases = sum(confirmed_cases, na.rm = TRUE),
            median_age = first(median_age),
            ...
            total_pop = first(total_pop)) %>%
  ungroup() %>%
  mutate(cases_per_100k = (confirmed_cases / total_pop) * 100000) %>%  # Normalize
  lm(cases_per_100k ~ median_age + median_income + total_pop, data = .)
