...
# Clean mobility to exclude holiday dates
cmd <- mobility_data |>
  filter(!is.na(retail_and_recreation_percent_change_from_baseline)) |>
  filter(country_region == "United States") |> 
  filter(sub_region_2 != "") |> # getting rid of blanks
  filter(sub_region_1 != "") |> 
  select(-metro_area, -iso_3166_2_code, -census_fips_code, -country_region_code) |> # removing unecessary columns
...
We can standardize county_fips_code and state_fips_Code 
...
We added two new columns called cases_growth_rate and death_growth_rate. These columns show the growth rate of the cases and deaths, respectively. The deaths do not show as high a growth rate as the cases. We can make a chart to show this. We want to look at the death and growth rate so that we can understand how government policy shaped covid.  
...
There are more cases than there are deaths.The growth rates for the cases vary by a significant amount, so we can capture finer changes by making another graph with a log scale. This log scale can show us smaller changes if we pick a small scale like 0.1% to 1000%. We can correlate this graph with real time events. Through this, we can understand how government policy shaped covid. 
...
renamed_tx_data <- subset(tx_data, select = c(county_name, confirmed_cases, deaths, median_age, median_income, total_pop))

# Assign new column names to the new data frame
colnames(renamed_tx_data) <- c("County", "Cases", "Deaths", "MedianAge", "MedianIncome", "Population")

head(renamed_tx_data, 5)
```
...
# rename tx_data_with_gr 
renamed_tx_data_with_gr <- subset(tx_data_with_gr, select = c(county_name, confirmed_cases, deaths, median_age, median_income, total_pop, date_of_first_case, rate_of_growth_first_30_days, rate_of_growth_30_60_days, rate_of_growth_60_90_days, rate_of_growth_90_120_days, rate_of_growth_120_150_days, rate_of_growth_150_180_days, infection_rate, death_rate))
# Assign new column names to the new data frame
colnames(renamed_tx_data_with_gr) <- c("County", "Cases", "Deaths", "MedianAge", "MedianIncome", "Population", "FirstCases", "30 days", "30-60 days", "60-90 days", "90-120 days", "120-150 days", "150-180 days", "Inf.Rate", "DeathRate")

head(renamed_tx_data_with_gr, 10)
